...
### Chained execution — when the formula needs the query engine's output first

Some questions aren't "query OR formula," they're "query THEN formula on
those exact rows" (e.g. *"blended efficiency index for my top 20 customers by
switch propensity"*). This is a pipeline, not two independent branches merged
at the end, so intent extraction returns an **ordered step list** instead of
a single flat spec:

```json
{
  "steps": [
    { "engine": "query",   "spec": {"sort_by": "switch_propensity_score", "sort_desc": true, "limit": 20} },
    { "engine": "formula", "formula_id": "blended_efficiency_index", "input": "step_1_result" }
  ]
}
```

The backend executes the steps in order, passing each step's resulting
DataFrame slice as the working `df` for the next step — the formula runs
only on the rows the query step returned, not the full portfolio. Every
step still goes through the same checks as §4a/§4 in isolation (row-level
security first, AST whitelist on any `filter_expr`, `required_columns`
check before a formula runs).

The order isn't always query-then-formula — "which customers have a blended
efficiency index above 80" needs the formula computed over the population
*first*, then a filter on that computed column, so the planner picks
whichever order the question implies.

**One rule specific to chaining:** only the *last* step in the list may apply
`select_columns` (trim down to display columns). An intermediate step that
trims columns can silently strip a column a later formula's
`required_columns` needs — so column selection is deferred until the final
step, right before the result goes to Context Assembly.
